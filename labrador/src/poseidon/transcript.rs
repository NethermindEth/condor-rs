...
use super::constants::PermutationConstants;

struct Poss31;

impl PermutationConstants for Poss31 {
    const WIDTH: usize = 4;
    const PARTIAL_ROUNDS: usize = 1;
    const FULL_ROUNDS: usize = 2;
    const ALPHA: u64 = 3;

    type ArkArray = [[Zq; Self::WIDTH]; Self::FULL_ROUNDS + Self::PARTIAL_ROUNDS];
    type MdsArray = [[Zq; Self::WIDTH]; Self::WIDTH];

    const ARK: Self::ArkArray = Self::generate_ARK(10);

    const MDS: Self::MdsArray = Self::generate_MDS();
}

impl Poss31 {
    fn generate_ARK(rounds: usize) -> [[Zq; Self::WIDTH]; Self::FULL_ROUNDS + Self::PARTIAL_ROUNDS] {
        // Create a matrix to store constants for each round and state position
        let mut ark = [[Zq::new(0); Self::WIDTH]; Self::FULL_ROUNDS + Self::PARTIAL_ROUNDS];
        ...
        for row in ark.iter_mut().take(rounds) {
            ...
            for state in row.iter_mut().take(Self::WIDTH) {
    ...
    const fn generate_MDS() -> [[Zq; Self::WIDTH]; Self::WIDTH] {
        todo!()
    }
}
