...
impl Projection {
    ...
    pub fn compute_batch_projection(&self, witness_vector: &[RqVector]) -> Vec<Zq> {
        ...
        for i in 0..self.multiplicity {
            for j in 0..2 * self.security_level {
                result[j] += self.compute_projection(i, &witness_vector[i])[j];
            }
        }
        result
    }
...
//     pub fn new(random_zq_vector: Vec<Zq>, security_parameter: usize, rank: usize) -> Self {
//         let mut matrix = vec![vec![Zq::ZERO; rank * Rq::DEGREE]; 2 * security_parameter];
//         for (row_idx, row) in matrix.iter_mut().enumerate() {
//             for (col_idx, element) in row.iter_mut().enumerate() {
//                 *element = if random_zq_vector[row_idx * rank * Rq::DEGREE + col_idx].get_value() < 2_u32.pow(29) {
//                     Zq::MAX // -1 in Zq
//                 } else if random_zq_vector[row_idx * rank * Rq::DEGREE + col_idx].get_value() < 2_u32.pow(30) {
//                     Zq::ONE
//                 } else {
//                     Zq::ZERO
//                 };
