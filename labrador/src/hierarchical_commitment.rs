...
impl<const M: usize, const N: usize, const D: usize> HierarchicalCommitment<M, N, D> {
    ...
    fn create_outer_witness(&self, parts: &[Rq<D>]) -> RqVector<N, D> {
        ...
        if parts.len() >= N {
            ...
            // Ensure we don't exceed size N
            combined_coeffs.truncate(N);
        }
        ...
        for poly in &combined_coeffs {
            ...
            for (i, coeff) in poly.get_coefficients().iter().enumerate() {
                // Apply bounds by taking modulo witness_bound
                let bounded_coeff = *coeff % witness_bound;
                // Use center representatives to minimize coefficient size
                let half_bound = witness_bound / Zq::TWO;
                let final_coeff = if bounded_coeff > half_bound {
                    bounded_coeff - witness_bound
                } else {
                    bounded_coeff
                };

                bounded_coeffs_array[i] = final_coeff;
            }
